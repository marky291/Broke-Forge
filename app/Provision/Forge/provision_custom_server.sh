...
OS_NOT_SUPPORTED="Forge only supports Ubuntu ${AVAILABLE_VERSIONS[@]}."
...
if [[ "$UNAME" != "Ubuntu" || ! " ${AVAILABLE_VERSIONS[@]} " =~ " ${VERSION} " ]]; then
  ...
  exit 1
    ...
    FORGE_UBUNTU_VERSION="24.04"

if [[ ! -f /etc/os-release ]]; then
  echo "Could not determine OS version. /etc/os-release file not found."
  exit 1
fi

VERSION=$(awk -F= '/^VERSION_ID/{print $2}' /etc/os-release | sed 's/\"//g')

if [[ "$FORGE_UBUNTU_VERSION" != "$VERSION" ]]; then
  echo "Forge Ubuntu version mismatch. Expected ubuntu:$FORGE_UBUNTU_VERSION but this server is ubuntu:$VERSION."
  echo "Please recreate the server in Forge and select $VERSION as the Ubuntu version."
  exit 1
fi

    apt-get install -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" -y --force-yes curl

provisionPing 960937 1
...
# Laravel Forge
ssh-rsa  worker@forge.laravel.com
...
response=`curl -s -w "%{http_code}" POST --url https://forge.laravel.com/servers/960937/vps/connection --data-urlencode "forge_token="`
...
provisionPing 960937 3
apt-get update
apt-get -o Dpkg::Options::="--force-confold" upgrade -y

    apt-get install -y --force-yes software-properties-common

# add-apt-repository -y -n ppa:fkrull/deadsnakes-python2.7
# add-apt-repository -y -n ppa:nginx/mainline
add-apt-repository -y -n ppa:ondrej/nginx
# add-apt-repository -y -n ppa:chris-lea/redis-server

add-apt-repository -y -n ppa:ondrej/php
add-apt-repository -y -n universe

add-apt-repository -y -n ppa:laravelphp/forge
...
echo "unwavering-berlin" > /etc/hostname
sed -i 's/127\.0\.0\.1.*localhost/127.0.0.1	unwavering-berlin.localdomain unwavering-berlin localhost/' /etc/hosts
hostname unwavering-berlin

# Set The Sudo Password For Forge

PASSWORD=$(mkpasswd -m sha-512 '9fl%[d.jxS)p1x.4__e]')
...
git config --global user.email "markhester.dev+laravelforge@pm.me"
# Setup UFW Firewall
...
npm install -g bun
    ...
    provisionPing 960937 6
    export DEBIAN_FRONTEND=noninteractive

# Add MySqlProvision Keys...

sudo apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 467B942D3A79BD29

# Configure MySqlProvision Repositories If Required
...
debconf-set-selections <<< "mysql-community-server mysql-community-server/root-pass password SzROnxNzjnVqqU5bthlt"
debconf-set-selections <<< "mysql-community-server mysql-community-server/re-root-pass password SzROnxNzjnVqqU5bthlt"

# Install MySqlProvision
...
[client]
user=root
password=SzROnxNzjnVqqU5bthlt
EOF

chmod 600 "$MYSQL_TEST_LOGIN_FILE"

mysql -e "CREATE USER 'root'@'127.0.0.1' IDENTIFIED BY 'SzROnxNzjnVqqU5bthlt';"
mysql -e "CREATE USER 'root'@'%' IDENTIFIED BY 'SzROnxNzjnVqqU5bthlt';"
mysql -e "GRANT ALL PRIVILEGES ON *.* TO root@'127.0.0.1' WITH GRANT OPTION;"
mysql -e "GRANT ALL PRIVILEGES ON *.* TO root@'%' WITH GRANT OPTION;"
service mysql restart

mysql -e "CREATE USER 'forge'@'127.0.0.1' IDENTIFIED BY 'SzROnxNzjnVqqU5bthlt';"
mysql -e "CREATE USER 'forge'@'%' IDENTIFIED BY 'SzROnxNzjnVqqU5bthlt';"
mysql -e "GRANT ALL PRIVILEGES ON *.* TO 'forge'@'127.0.0.1' WITH GRANT OPTION;"
mysql -e "GRANT ALL PRIVILEGES ON *.* TO 'forge'@'%' WITH GRANT OPTION;"
mysql -e "FLUSH PRIVILEGES;"

# Create The Initial Database If Specified

mysql -e "CREATE DATABASE forge CHARACTER SET utf8 COLLATE utf8_unicode_ci;"
...
if [[ $(grep --count "maxsize" /etc/logrotate.d/mysql-server) == 0 ]]; then
    ...
    # If MySqlProvision Fails To Start, Re-Install It
...
curl --insecure --data "event_id=128289375&server_id=960937&recipe_id=" https://forge.laravel.com/provisioning/callback/app
