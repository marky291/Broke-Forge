...
namespace App\Packages\Services\Sites\Framework\GenericPhp;
...
/**
 * Generic PHP Installation Job
 *
 * Handles queued Generic PHP site installation on remote servers with real-time status updates.
 */
class GenericPhpInstallerJob extends BaseFrameworkInstaller
{
    /**
     * Get framework-specific installation steps for Generic PHP.
     */
    protected function getFrameworkSteps(ServerSite $site): array
    {
        ...
        $steps = array_merge($steps, [
            ['name' => 'Creating environment file', 'description' => 'Generating .env configuration'],
    ...
    /**
     * Execute Generic PHP-specific installation logic.
     */
    protected function installFramework(ServerSite $site): void
    {
        ...
        $this->updateInstallationStep($site, $currentStep, TaskStatus::Installing);
        $this->executeCommands([
            sprintf('touch %s/.env', escapeshellarg($deploymentPath)),
        ]);
        $this->updateInstallationStep($site, $currentStep++, TaskStatus::Success);

        // Step 5: Install Composer dependencies (if there's a composer.json)
        $this->updateInstallationStep($site, $currentStep, TaskStatus::Installing);
        if (isset($site->configuration['git_repository'])) {
            $this->executeCommands([
                sprintf(
    ...
    {
        return 'Generic PHP installation';
    }
}
