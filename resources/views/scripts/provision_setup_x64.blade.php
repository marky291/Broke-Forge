...
CALLBACK_STEP_URL='{!! $callbackUrls['step'] ?? '' !!}'

notify_step() {
  local step="$1"
  local status="$2"

  if [ -z "${CALLBACK_STEP_URL}" ]; then
    echo "[!] WARNING: No callback URL provided"
    return
  fi

  echo "[→] Updating step ${step} to status: ${status}..."

  if command -v curl >/dev/null 2>&1; then
    if curl -L -X POST "${CALLBACK_STEP_URL}" -d "step=${step}" -d "status=${status}" >/dev/null 2>&1; then
      echo "[✓] Step ${step} updated successfully"
    else
      echo "[!] WARNING: Step ${step} callback failed (curl exit code: $?)"
    fi
  elif command -v wget >/dev/null 2>&1; then
    if wget --method=POST --body-data="step=${step}&status=${status}" -qO- "${CALLBACK_STEP_URL}" >/dev/null 2>&1; then
      echo "[✓] Step ${step} updated successfully"
    else
      echo "[!] WARNING: Step ${step} callback failed (wget exit code: $?)"
    fi
  else
