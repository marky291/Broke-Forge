    ...
    if (!paymentMethods || paymentMethods.length === 0) {
        return (
            <div className="py-8 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Add a payment method to subscribe to a plan</p>
            </div>
        );
    }
            ...
            {paymentMethods.map((method) => (
                <div
                    ...
                    className={cn(
                        ...
                        method.is_default ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50',
                        method.is_expired && 'opacity-60',
                    )}
                >
                    <div className="flex items-center gap-3">
                        <div
                            className={cn('flex size-10 items-center justify-center rounded-full', method.is_default ? 'bg-primary/10' : 'bg-muted')}
                        >
                            <CreditCard className={cn('size-5', method.is_default ? 'text-primary' : getBrandIcon(method.brand))} />
                        </div>
                        <div>
