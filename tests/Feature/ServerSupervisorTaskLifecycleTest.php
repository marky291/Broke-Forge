...
class ServerSupervisorTaskLifecycleTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_controller_creates_task_with_pending_status(): void
    {
        ...
        $response = $this->post(route('servers.supervisor.tasks.store', $server), [
            'name' => 'Test Task',
        ...
        $task = ServerSupervisorTask::where('name', 'Test Task')->first();
        $this->assertNotNull($task);
        $this->assertEquals('pending', $task->status->value);
        $this->assertEquals('php artisan queue:work', $task->command);
    }
    ...
    public function test_controller_dispatches_job_with_task_id(): void
    {
        ...
        $this->post(route('servers.supervisor.tasks.store', $server), [
            'name' => 'Test Task',
            'command' => 'php artisan queue:work',
            'working_directory' => '/home/brokeforge',
            'processes' => 1,
        ...
        Queue::assertPushed(SupervisorTaskInstallerJob::class, function ($job) use ($server, $task) {
    ...
    public function test_job_updates_status_to_installing(): void
    {
        ...
        $user = User::factory()->create();
        $server = Server::factory()->for($user)->create();
        $task = ServerSupervisorTask::factory()->for($server)->create([
            'status' => 'pending',
        ]);
        ...
        try {
            $job = new SupervisorTaskInstallerJob($server, $task->id);
            $job->handle();
        } catch (\Exception $e) {
    ...
    public function test_retry_resets_failed_task_to_pending_and_dispatches_job(): void
    {
        ...
        $response = $this->post(route('servers.supervisor.tasks.retry', [$server, $task]));
    ...
    public function test_retry_only_works_for_failed_tasks(): void
    {
        ...
        Queue::assertNotPushed(SupervisorTaskInstallerJob::class);
    }
    ...
    public function test_controller_sets_status_to_removing_before_dispatching_removal_job(): void
    {
        ...
        $response = $this->delete(route('servers.supervisor.tasks.destroy', [$server, $task]));
        ...
        Queue::assertPushed(SupervisorTaskRemoverJob::class, function ($job) use ($server, $task) {
            return $job->server->id === $server->id
                && $job->taskId === $task->id;
        });
    }
    ...
    public function test_removal_job_deletes_task_on_success(): void
    {
        ...
        $job = new SupervisorTaskRemoverJob($server, $task->id);
        $job->handle();
