...
namespace Tests\Unit\Packages\Services\Sites\Framework\StaticHtml;
...
class StaticHtmlInstallerJobTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_job_has_correct_timeout_property(): void
    {
        ...
        $job = new StaticHtmlInstallerJob($server, $site->id);
    ...
    public function test_constructor_accepts_server_and_site_id(): void
    {
        ...
        // Assert
        $this->assertInstanceOf(StaticHtmlInstallerJob::class, $job);
        $this->assertEquals($server->id, $job->server->id);
        $this->assertEquals($site->id, $job->siteId);
    }

    /**
     * Test nginx configuration properly escapes double quotes for Static HTML sites.
     */
    public function test_nginx_configuration_escapes_double_quotes(): void
    {
        ...
        $site = ServerSite::factory()->create([
            'server_id' => $server->id,
            'domain' => 'static.test',
            'document_root' => '/home/brokeforge/static.test/public',
            'php_version' => '8.2',
            'ssl_enabled' => false,
        ]);
        $job = new StaticHtmlInstallerJob($server, $site->id);
