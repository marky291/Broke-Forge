...
class ServerMonitoringInstallerJobTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_job_has_correct_timeout_property(): void
    {
        ...
        $job = new ServerMonitoringInstallerJob($server);
    ...
    public function test_constructor_accepts_parameters(): void
    {
        $server = Server::factory()->create();
        $job = new ServerMonitoringInstallerJob($server);
        $this->assertInstanceOf(ServerMonitoringInstallerJob::class, $job);
