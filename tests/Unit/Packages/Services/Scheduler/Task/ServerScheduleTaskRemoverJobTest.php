...
class ServerScheduleTaskRemoverJobTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_job_has_correct_timeout_property(): void
    {
        ...
        $job = new ServerScheduleTaskRemoverJob($server, $record->id);
    ...
    public function test_constructor_accepts_server_and_id(): void
    {
        $server = Server::factory()->create();
        $recordId = 123;
        $job = new ServerScheduleTaskRemoverJob($server, $recordId);
        $this->assertInstanceOf(ServerScheduleTaskRemoverJob::class, $job);
    ...
    public function test_failed_method_handles_missing_records_gracefully(): void
    {
        $server = Server::factory()->create();
        $nonExistentId = 99999;
        $job = new ServerScheduleTaskRemoverJob($server, $nonExistentId);
